...
# Run the job inside the docker container (GPU ID set by submit script, e.g. 1 and 2)
NV_GPU={{GPU_ID}}

# Initialize log file
LOG_FILE="{{DATE}}_{{REPL}}.log"
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ] && [ $EXIT_CODE -ne 0 ]; do
    ...
    docker run \
        ...
        --rm \
        -v ${BASE_DIR}/runs/{{DATE}}_{{REPL}}:/mnt \
        protein_science \
        bash -c "cd /mnt/scripts/ && if [ '$IS_RESTART' != 'true' ]; then bash run_sc_chain_generation.sh; fi && python3 run_btree_chromo_replicate.py {{SEED}} {{DATE}}_{{REPL}} $START_TIME $END_TIME $IS_RESTART {{V}}" >> "$LOG_FILE" 2>&1
