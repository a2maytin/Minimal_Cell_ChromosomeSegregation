...
def create_directives(run_name, seed, timestep, v_bps=200):
    """
    Creates a directives file for btree_chromo by reading a template
    and replacing placeholders based on the timestep.
    v_bps: translocation speed in bp/s; template value is v_bps/10 (e.g. 200 -> 20).
    """
    sim_prng_seed = seed
    translocate_speed = int(round(v_bps / 10.0))
    ...
    template_file = template_dir + "template_replicate.inp"
    fudge_factor = 160
    ...
    run_dynamics = "simulator_run_soft_harmonic:20000,1000,40000,append,nofirst"
    sphere_height = 0
    ...
    add_ribosomes_to_bin(ribos_path, 5, sphere_radius, sphere_height, n_normalized)
    ...
    directives_content = directives_content.format(
        ...
        translocate_speed=translocate_speed
    )
...
def main():
    ...
    # Translocation speed in bp/s (e.g. 200 -> translocate:20,T in template). Default 200.
    v_bps = int(sys.argv[6]) if len(sys.argv) > 6 else 200
    ...
    print(f"Start time: {start_time}, End time: {end_time}, Restart: {is_restart}")

    for step in range(start_time, end_time + 1, 1):
       directives_file = create_directives(run_name, seed, step, v_bps=v_bps)
